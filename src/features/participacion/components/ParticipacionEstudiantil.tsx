...
import { useProcedencia, useGenero, useEstrato } from '../hooks/useParticipacion';
...
export function ParticipacionEstudiantil() {
  const [periodo, setPeriodo] = useState('');

  const { data: estrato, isLoading: loadingEstrato, refetch: refetchEstrato } = useEstrato(periodo || undefined);
  ...
  const isLoading = loadingEstrato || loadingProc || loadingGen;
  const total = estrato?.total_estudiantes ?? 0;

  const handleRefresh = () => { refetchEstrato(); refetchProc(); refetchGen(); };
  ...
  return (
    <div className="space-y-8 fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold font-display text-brand-primary">Participación Estudiantil</h2>
          <p className="text-slate-500 text-sm mt-0.5">Análisis de participación y caracterización del estamento estudiantil</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <ChartCard
          ...
          isLoading={loadingEstrato}
        >
          <ResponsiveContainer width="100%" height={220}>
            <BarChart
              data={estrato?.por_estrato ?? []}
          ...
          {estrato && estrato.agrupado.length > 0 && (
            <div className="mt-4 flex gap-3 flex-wrap">
              {estrato.agrupado.map(g => (
