...
import { FileText, Calendar, Plus, Eye, Send, Lock, Trash2, Share2, BarChart2, Users } from 'lucide-react';
  ...
  return (
    <div className="space-y-8 fade-in">
      ...
      <Modal
        ...
        title={resultadosData?.titulo ?? 'Resultados de la encuesta'}
        description={`${resultadosData?.total_respuestas ?? 0} respuesta(s) recibidas`}
        className="max-w-2xl"
      >
        {isLoadingResultados ? (
          ...
          <div className="space-y-5 max-h-[60vh] overflow-y-auto pr-1">
            {resultadosData.resultados_por_pregunta?.length === 0 && (
              <p className="text-sm text-slate-500 text-center py-6">No hay respuestas registradas aún.</p>
            )}
            {resultadosData.resultados_por_pregunta?.map((r: any, idx: number) => (
